...
export default function TrainLineDetail() {
  ...
  const lineColor = (statusInfo as any).color || (lineData[lineId] && (lineData[lineId] as any).color) || '#43a047';

  // ステータスに応じた色とアイコン
  let statusColor = '#43a047';
  let statusIcon = <Box sx={{ width: 18, height: 18, borderRadius: '50%', border: '2.5px solid #43a047', display: 'inline-block', mr: 1 }} />;
  if (status === '遅延') {
    statusColor = '#ffa000';
    statusIcon = (
      <Box sx={{ width: 18, height: 18, display: 'inline-block', mr: 1 }}>
    ...
    statusColor = '#e53935';
    statusIcon = (
      <Box sx={{ width: 18, height: 18, display: 'inline-block', mr: 1, position: 'relative' }}>

...
export async function GET(request: Request) {
  ...
  const type = requestUrl.searchParams.get('type') // メール確認の場合は 'recovery' または 'signup'
  ...
  console.log('Type:', type) // メール確認の種類
  ...
  if (code) {
    ...
    try {
      ...
      if (type === 'recovery' || type === 'signup') {
        ...
        if (exchangeError) {
          console.error('❌ Email confirmation code exchange error:', exchangeError)
        ...
        // メール確認成功後の処理
        if (data.user) {
          console.log('👤 Creating/updating user profile after email confirmation...')
          const { username, game_tag } = data.user.user_metadata || {}
          
          let displayName = username || data.user.email?.split('@')[0] || `user_${data.user.id.slice(0, 8)}`
          let gameTag = game_tag || data.user.email?.split('@')[0] || `user_${data.user.id.slice(0, 8)}`
          ...
          try {
            ...
            if (profileError) {
              ...
              console.log('✅ Profile created/updated successfully after email confirmation')
            }
          } catch (profileError) {
            console.error('❌ Profile creation exception after email confirmation:', profileError)
          }
        }

...
## AIC所属シート（正社員1社・アルバイト複数）

ユーザーの所属は **正社員1社まで**・**アルバイトは複数可**です。AIC所属シートは次の列です（既存の3列形式も読み取り対応）。

| 列 | 内容 |
|----|------|
| A | user_id |
| B | main_company_name（正社員としての所属1社） |
| C | part_time_companies（アルバイト所属の会社名を JSON 配列で。例: `["会社A","会社B"]`） |
| D | updated_at |

初回はヘッダーが自動作成されます。既存の「user_id, company_name, updated_at」3列のシートがある場合は、B列を正社員の会社名として読み、アルバイトは空として扱います。

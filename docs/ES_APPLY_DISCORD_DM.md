...
4. 社長が **ダッシュボード（自分の投稿）** を開き、該当申請の **✅（許可）** または **❌（拒否）** を押すと、申請ステータスが更新される。許可の場合は AIC 所属も更新される。**許可した場合、申請者（応募者）の Discord DM に「入社承認のご連絡」が送られる**（申請時に Discord 連携でログインしていて `discord_id` が記録されている場合のみ）。

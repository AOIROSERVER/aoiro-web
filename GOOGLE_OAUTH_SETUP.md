...
**原因**: Googleログイン後のセッション設定でエラーが発生
**解決方法**:
1. ブラウザのキャッシュをクリア（Ctrl+Shift+Delete）
2. ローカルストレージをクリア
3. `/test-google-oauth`ページでテストを実行
4. 開発者ツール（F12）のコンソールでエラーを確認
...
**原因**: Googleの認証コードが無効または期限切れ
**解決方法**:
1. ブラウザのキャッシュをクリア
2. ローカルストレージをクリア
3. 再度ログインを試行

#### 問題: "unauthorized_client" エラー
**原因**: Googleのクライアント認証に失敗
**解決方法**:
1. Google Cloud ConsoleでクライアントIDとシークレットを再確認
2. Supabaseで設定を再確認
3. 必要に応じてクライアントシークレットを再生成
